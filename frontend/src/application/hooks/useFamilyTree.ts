...
import { isAutogeneratedCardId } from '../utils/filter';
import { PersonData } from '../types';
import { FamilyCard, HEIGHT_PX, WIDTH_PX } from '../components/FamilyCard';

export const useFamilyTree = (onOpenAdminModal?: (url: string) => void) => {
      ...
      try {
        ...
        const modifyTreeHierarchy = (node: any, isMainNode: boolean) => {
          ...
          const updateChildren = (node: any) => {
            ...
            node.children = node.children.filter((person: any) => !isAutogeneratedCardId(person.data.id));
            node.children.forEach((child: any) => updateChildren(child));
          };
          updateChildren(node);
        };
        ...
        const Card = (tree: any, svg: any) => {
          return function (d: any) {
            ...
            if (isAutogeneratedCardId(d.data.id)) {
              root.render(null);
              return;
            }
